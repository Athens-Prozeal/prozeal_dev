...
const
BusductDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const
  busductId = searchParams.get('busductId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/busduct/${busductId}/?work_site_id=${workSiteId}`, {
        headers: {
            ...
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR BUSDUCT
                </Typography>
              </Box>
            </Stack>
          </Grid>
