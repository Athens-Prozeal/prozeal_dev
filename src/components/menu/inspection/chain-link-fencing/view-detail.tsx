...
const ChainLinkFencingDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const chainLinkFencingId = searchParams.get('ChainLinkFencingId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/chain-link-fencing/${chainLinkFencingId}/?work_site_id=${workSiteId}`, {
        headers: {
    ...
    <Box display="flex" justifyContent="center" minHeight="100vh" flexDirection="column" gap={4}>
      <Paper elevation={2} sx={{ maxWidth: '100%', width: '100%' }}>
        <Grid container mb={2} sx={{ border: '1px solid #999999', minHeight: '130px' }}>
          ...
          <Grid item xs={6}>
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                   CHECKLIST FOR PLANT BOUNDARY AND FENCING Chain-Link-Fencing
        ...
        <Grid item xs={12} sm={6}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Drawing No:</strong> {data?.drawing_no}
            </Typography>
          </Grid>
...
export default ChainLinkFencingDetail;
