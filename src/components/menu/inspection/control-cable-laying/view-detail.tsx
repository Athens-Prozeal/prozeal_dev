...
const ControlCableLayoutDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const controlCableLayingId = searchParams.get('controlCableLayingId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/control-cable-laying/${controlCableLayingId}/?work_site_id=${workSiteId}`, {
        headers: {
        ...
        <Grid container mb={2} sx={{ border: '1px solid #999999', minHeight: '130px' }}>
          ...
          <Grid item xs={6}>
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR CONTROL CABLE LAYING
                </Typography>
              </Box>
            </Stack>
          </Grid>
...
export default ControlCableLayoutDetail;
