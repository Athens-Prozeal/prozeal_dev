  ...
  export const Form = () => {
    ...
    const onSubmit = async (data: GridEathingSchemaType) => {
      ...
      if (validateForm()) {
        ...
        axios({
          method: 'POST',
          url: `${config.site.serverURL}/api/inspection/grid-earthing/?work_site_id=${localStorage.getItem('work-site-id')}`,
          data: {
          ...
          .then((response) => {
            if (response.status === 201) {
              ...
              setTimeout(() => {
                window.location.href = '/menu/inspection/grid-earthing?status=approved';
              }, 500);
            }
          })
          .catch((error) => {
