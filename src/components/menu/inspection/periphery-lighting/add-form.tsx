...
const checklistSchemas: ChecklistSchema = {
    Check_the_correctness_of_foundation_and_its_foundation_bolt: {
    verbose_name: 'Check the correctness of foundation and its foundation bolt',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_that_light_pole_must_be_vertically_straight_and_cross_arm_is_fixed_properly: {
    verbose_name: 'Check that light pole must be vertically straight and cross arm is fixed properly',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_cable_make_size_voltage_grade_conductor_and_insulation_type_as_per_design_specifications: {
    verbose_name: 'Check cable make, size, voltage grade, conductor & insulation type as per design/specifications',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_Cable_route_as_per_drawings: {
  ...
  Check_for_proper_Dressing_of_the_cables: {
    verbose_name: 'Check for proper Dressing of the cables',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_for_the_installation_of_caution_danger_tape_as_per_approved_drawings: {
    verbose_name: 'Check for the installation of caution / danger tape as per approved drawings',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Cable_Jointin_and_terminations_done_as_per_manufacturers_instruction_manual: {
    verbose_name: 'Cable Jointing & terminations done as per manufacturers instruction manual',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_tightness_of_the_cable_terminations_at_connection_points: {
    verbose_name: 'Check tightness of the cable terminations at connection points',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_for_Earthing_Connections_provided: {
  ...
  Check_that_lighting_fixture_and_lamp_are_fixed_properly_and_working: {
    verbose_name: 'Check that lighting fixture and lamp are fixed properly and working',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
  Check_that_Grouting_of_lighting_pole_must_be_with_non_compressive_cement_motor: {
    verbose_name: 'Check that Grouting of lighting pole must be with non-compressive cement motor',
    choices: ['Yes', 'No', 'N/A'],
    required: true,
  },
...
const peripheryLightingSchema = z.object(basePeripheryLightingSchema);

type PeripheryLightingSchemaType = z.infer<typeof peripheryLightingSchema>;
...
export const Form = () => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<PeripheryLightingSchemaType>({
    resolver: zodResolver(peripheryLightingSchema),
  });

  const onSubmit = async (data: PeripheryLightingSchemaType) => {
    if (validateForm()) {
      ...
      axios({
        method: 'POST',
        url: `${config.site.serverURL}/api/inspection/periphery-lighting/?work_site_id=${localStorage.getItem('work-site-id')}`,
        data: {
        ...
        .then((response) => {
          if (response.status === 201) {
            window.alert('Periphery Lighting Report Added');
            setTimeout(() => {
              window.location.href = '/menu/inspection/periphery-lighting?status=approved';
            }, 500);
          }
        })
        .catch((error) => {
