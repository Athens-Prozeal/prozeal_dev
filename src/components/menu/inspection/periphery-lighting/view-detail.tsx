...
const PeripheryLightingDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const peripheryLightingId = searchParams.get('peripheryLightingId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/periphery-lighting/${peripheryLightingId}/?work_site_id=${workSiteId}`, {
        headers: {
    ...
    <Box display="flex" justifyContent="center" minHeight="100vh" flexDirection="column" gap={4}>
      <Paper elevation={2} sx={{ maxWidth: '100%', width: '100%' }}>
            ...
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR PERIPHERY LIGHTING
                </Typography>
              </Box>
            </Stack>
          </Grid>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Block No:</strong> {data?.block_no}
            </Typography>
          </Grid>
...
export default PeripheryLightingDetail;
