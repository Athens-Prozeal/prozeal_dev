...
const plainCementConcreteWorkDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const plainCementConcreteWorkId = searchParams.get('plainCementConcreteWorkId');
  ...
  useEffect(() => {
    axios
      .get(
        `${config.site.serverURL}/api/inspection/plain-cement-concrete-work/${plainCementConcreteWorkId}/?work_site_id=${workSiteId}`,
        {
          headers: {
    ...
    <Box display="flex" justifyContent="center" minHeight="100vh" flexDirection="column" gap={4}>
      <Paper elevation={2} sx={{ maxWidth: '100%', width: '100%' }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Grade/ Mix proportion of concrete:</strong> {data?.grade_mix_pro_portion_of_concrete}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
...
export default plainCementConcreteWorkDetail;
