...
const PlantBoundaryAndFencingDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const plantBoundaryAndFencingId = searchParams.get('plantBoundaryAndFencingId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/plant-boundary-and-fencing/${plantBoundaryAndFencingId}/?work_site_id=${workSiteId}`, {
        headers: {
    ...
    <Box display="flex" justifyContent="center" minHeight="100vh" flexDirection="column" gap={4}>
      <Paper elevation={2} sx={{ maxWidth: '100%', width: '100%' }}>
        <Grid container mb={2} sx={{ border: '1px solid #999999', minHeight: '130px' }}>
          ...
          <Grid item xs={6}>
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR PLANT BOUNDARY AND FENCING (RCC)
                </Typography>
              </Box>
            </Stack>
          </Grid>
        ...
        <Grid item xs={12} sm={6}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Project Name & Capacity:</strong> {data?.project_name_and_capacity}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Consultant Name:</strong> {data?.consultant_name}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>EPC Contractor Name:</strong> {data?.epc_contractor_name}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Location / Area:</strong> {data?.location_or_area}
            </Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Contractor Name:</strong> {data?.contractor_name}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Work Supervisor Name:</strong> {data?.work_supervisor_name}
            </Typography>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Any Other Observation:</strong> {data?.any_other_observation}
            </Typography>
          </Grid>
...
export default PlantBoundaryAndFencingDetail;
