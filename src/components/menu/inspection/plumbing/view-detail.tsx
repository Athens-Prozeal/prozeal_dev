...
const PlumbingDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const plumbingId = searchParams.get('plumbingId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/plumbing/${plumbingId}/?work_site_id=${workSiteId}`, {
        headers: {
            ...
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR Plumbing
                </Typography>
              </Box>
            </Stack>
          </Grid>
