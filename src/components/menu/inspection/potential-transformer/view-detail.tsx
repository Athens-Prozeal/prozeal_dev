...
const PotentialTransformerDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const potentialTransformerId = searchParams.get('potentialTransformerId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/potential-transformer/${potentialTransformerId}/?work_site_id=${workSiteId}`, {
        headers: {
        ...
        <Grid container mb={2} sx={{ border: '1px solid #999999', minHeight: '130px' }}>
          ...
          <Grid item xs={6}>
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR POTENTIAL TRANSFORMER
                </Typography>
              </Box>
            </Stack>
          </Grid>
...
export default PotentialTransformerDetail;
