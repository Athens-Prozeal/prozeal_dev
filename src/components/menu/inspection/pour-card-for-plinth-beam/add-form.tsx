  ...
  export const Form = () => {
    ...
    const onSubmit = async (data: PourCardForPlinthBeamSchemaType) => {
      ...
      if (validateForm()) {
        ...
        axios({
          method: 'POST',
          url: `${config.site.serverURL}/api/inspection/pour-card-for-plinth-beam/?work_site_id=${localStorage.getItem('work-site-id')}`,
          data: {
          ...
          .then((response) => {
            if (response.status === 201) {
              ...
              setTimeout(() => {
                window.location.href = '/menu/inspection/pour-card-for-plinth-beam?status=approved';
              }, 500);
            }
          })
          .catch((error) => {
