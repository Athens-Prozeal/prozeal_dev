...
const checklistSchemas: ChecklistSchema = {
    ...
    const baseRCCSchema = {
        ...
        location_or_area: z.string().max(255, 'Location / Area must be at most 255 characters'),
      ...
      const RCCSchema = z.object(baseRCCSchema);

      type RCCSchemaType = z.infer<typeof RCCSchema>;
      ...
      export const Form = () => {
        ...
        const {
          ...
          handleSubmit,
          formState: { errors },
        } = useForm<RCCSchemaType>({
          resolver: zodResolver(RCCSchema),
        });

        const onSubmit = async (data: RCCSchemaType) => {
          if (validateForm()) {
            ...
            axios({
              method: 'POST',
              url: `${config.site.serverURL}/api/inspection/rcc/?work_site_id=${localStorage.getItem('work-site-id')}`,
              data: {
                project_name: data.projectName,
                customer: data.customer,
                location_or_area: data.location_or_area,
              ...
              .then((response) => {
                if (response.status === 201) {
                  window.alert('RCC Report Added');
                  setTimeout(() => {
                    window.location.href = '/menu/inspection/rcc?status=approved';
                  }, 500);
                }
              })
              .catch((error) => {
