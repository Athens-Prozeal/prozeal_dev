...
const ScadaSystemDetail: React.FC = () => {
  const searchParams = useSearchParams();
  const workSiteId = localStorage.getItem('work-site-id');
  const scadaSystemId = searchParams.get('scadaSystemId');
  ...
  useEffect(() => {
    axios
      .get(`${config.site.serverURL}/api/inspection/scada-system/${scadaSystemId}/?work_site_id=${workSiteId}`, {
        headers: {
    ...
    <Box display="flex" justifyContent="center" minHeight="100vh" flexDirection="column" gap={4}>
      <Paper elevation={2} sx={{ maxWidth: '100%', width: '100%' }}>
            ...
            <Stack direction="column" height="100%">
              ...
              >
                <Typography variant="h6" sx={{ fontWeight: 700 }}>
                  INSTALLATION CHECKLIST FOR SCADA SYSTEM
                </Typography>
              </Box>
            </Stack>
          </Grid>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <Typography variant="body1" sx={{ fontSize: { xs: '14px', sm: '16px' }, marginBottom: 2 }}>
              <strong>Comments/Remark:</strong> {data?.comments_or_remarks}
            </Typography>
          </Grid>
...
export default ScadaSystemDetail;
