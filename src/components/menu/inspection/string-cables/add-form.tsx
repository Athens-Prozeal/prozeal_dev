  ...
  export const Form = () => {
    ...
    const onSubmit = async (data: StringCablesSchemaType) => {
      if (validateForm()) {
        ...
        axios({
          method: 'POST',
          url: `${config.site.serverURL}/api/inspection/string-cables/?work_site_id=${localStorage.getItem('work-site-id')}`,
          data: {
          ...
          .then((response) => {
            if (response.status === 201) {
              window.alert('String Cable Report Added');
              setTimeout(() => {
                window.location.href = '/menu/inspection/string-cables?status=approved';
              }, 500);
            }
          })
          .catch((error) => {
